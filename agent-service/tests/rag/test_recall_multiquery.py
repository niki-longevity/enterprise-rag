"""
RAG召回测试脚本 - 多Query扩展版本
对每个query的多个扩展版本分别执行检索，合并去重后对比期望chunk，输出命中率和详细结果
...
def multi_query_search(expanded_queries, top_k=3):
    """
    对多个扩展查询分别检索，合并去重

    Args:
        expanded_queries: 扩展查询列表，如 ["查询1", "查询2", "查询3"]
        top_k: 每次检索返回的chunk数量

    Returns:
        merged_chunks: 合并去重后的chunk列表，格式[(file_name, chunk_idx), ...]
                      按首次出现的顺序排列
    """
    seen = set()
    merged_chunks = []
...
def run_recall_test_for_category(test_cases, category_name, top_k=3):
    """
    执行指定类别的召回测试（多Query扩展版）

    Args:
        test_cases: 测试用例列表，格式: [(original_query, [expanded_queries], expected_chunks), ...]
        category_name: 类别名称
        top_k: 每次检索返回的chunk数量
    """
    ...
    for original_query, expanded_queries, expected_chunks in test_cases:
        ...
        # 多Query检索，合并去重
        merged_chunks = multi_query_search(expanded_queries, top_k)
        actual_set = set(merged_chunks)
        ...
        # 计算MRR
        reciprocal_rank, best_rank = calculate_mrr_for_query(expected_chunks, merged_chunks)
    ...
    # 输出汇总
    print("=" * 60)
    print(f"RAG召回测试报告（多Query扩展） - {category_name} (top_k={top_k})")
    print("=" * 60)
    print(f"总测试数: {total}")
    ...
    # 平均合并后chunk数
    avg_retrieved = sum(d["total_retrieved"] for d in details) / total
    print(f"平均合并后chunk数: {avg_retrieved:.1f}")
    ...
    # 输出平均期望chunk数
    avg_expected = sum(len(case[2]) for case in test_cases) / total
    ...
    for d in details:
        if d["status"] != "ALL_HIT":
            print(f"\n[{d['status']}] Q: {d['original_query']}")
            print(f"  扩展查询: {d['expanded_queries']}")
            print(f"  期望: {d['expected']}")
            print(f"  命中: {d['matched']}")
            print(f"  缺失: {d['missed']}")
            print(f"  合并后结果({d['total_retrieved']}个): {d['actual']}")
...
def run_comprehensive_recall_test(top_k=5):
    """
    执行全面的召回测试（多Query扩展版），包括所有三类问题
    """
    print("=" * 60)
    print("RAG系统综合召回测试报告（多Query扩展）")
    print("=" * 60)
    print()
    ...
    avg_retrieved_all = sum(d["total_retrieved"] for d in all_details) / total_all
    ...
    print(f"  总体平均合并后chunk数: {avg_retrieved_all:.1f}")
    ...
    for i, (query, expanded, expected) in enumerate(complex_test_cases):
    ...
    chunk_counts = [len(expected) for _, _, expected in complex_test_cases]
